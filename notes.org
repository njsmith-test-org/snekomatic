...
- The app's own repo has to live inside the org being managed (b/c we need permissions to trigger github actions on it)
...
- [ ] get revision id from the trigger payload
- [ ] move into proper repo
- [ ] trigger a job from snekomatic
